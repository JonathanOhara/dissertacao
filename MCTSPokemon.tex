...
Quando ambos jogadores escolhem uma ação, a decisão de quem irá a executar a ação primeiro ocorre de maneira complexa. Simplificando, a ordem de qual ação acontece primeiro é dado pela seguinte prioridade (números menores mais prioridade):
...
Uma das customizações mais importantes são quais golpes o Pokémon pode utilizar. Cada Pokémon pode escolher 4 golpes entre dezenas de golpes disponíveis para sua espécie. Cada espécie de Pokémon pode aprender um conjunto diferente de habilidades dependendo do seu tipo e características (por exemplo, apenas Pokémon com chifre conseguem aprender o golpe \textit{Mega Horn}). Existem mais de 600 golpes diferentes.
...
onde \textbf{Type} é o modificador baseado na vantagem, explicado na subseção anterior, os valores podem variar entre  0, 0.25, 0.5, 1, 2, e 4. \textbf{Critical} é modificador caso o golpse seja crítico, normalmente um golpe tem chance de crítico de 6.25\%, caso esse acerto crítico ocorra o valor para \textbf{Critical} é de 1.5, caso contrário o valor é 1. \textbf{Other} são outros modificadores que podem ser adicionados por causa de itens ou estados especiais que modifiquem o dano. \textbf{Random} é um valor aleatório entre 85\% e 100\%.
...
\begin{itemize}
	...
	\item \textbf{Random battle} os dois jogadores são pareados com times totalmente aleatórios. Para balancear esse modo são atribuídos diferentes níveis de acordo com o \textit{tier} do Pokémon. Pokémon NU batalham no nível 86, RU 82, UU 78, OU 74 e \textit{Ubers} no nível 70. Os Pokémon do \textit{tier} PU não são utilizados nesse modo.
...
onde \textbf{V} é a variação do Elo. A constante \textbf{K} regula a variação do Elo, ou seja, valores grandes para K fará com que cada partida a variação de Elo seja maior (para mais e para menos), o Pokémon Showdown! utiliza o valor de K como 50. A variável \textbf{score} é o resultado da partida, é usado valor 0 para derrotas, 0.5 para empates e 1 para vitória. Já \textbf{foeElo} e \textbf{elo} são os valores de Elo do adversário e seu respectivamente.
...
\begin{itemize}
	...
	\item \textbf{waitForBattleRequest}. Método para ficar no menu inicial aceitando qualquer batalha, ele recebe como argumento uma função para ser executada quando a batalha começar.
...
\begin{itemize}
	...
	\item \textbf{getBattleCommands}. Método não recebe como argumentos e retorna um objeto do tipo \textit{BattleCommands}. Esse objeto pode conter 3 propriedades/métododos:
...
O objeto \textit{BattleStatus} tem mais 3 grandes objetos dentro dele como mostra a figura \ref{fig:showdownAIBattleMethods2}. As propriedades dele são:

\begin{itemize}

	\item \textbf{actions}. Retorna um objeto com os movimentos que o Poklémon ativo pode utilizar e os Pokémon disponíveis para usar.
...
Depois de criada a batalha ainda é possível alterar valores do objeto de batalha para melhor refletir a situação atual. Algumas propriedades que normalmente são necessários fazer ajustessão : quantidade de HP atual de cada Pokémon e adicionar estados especiais de batalhas.
...
Nos 4 agentes utiliziram técnicas de MCTS (explicados em \ref{sec:MCTS}), em alguns casos com modificações para se adequar ao teste proposto.

\subsection{Agentes para Random Battle}

Nesses dois primeiros agentes foram utilizados apenas a \textbf{showdownAI}, ou seja, não foram utilizados elementos de simulação durante o algoritmo. A ideia desses agentes é descobrir qual tipo de jogadas os agentes priorizam em certos momentos da batalha, ou seja, sem ter conhecimento nenhum sobre o jogo, descobrir em cada turno que se passa se é melhor utilizar: golpes fortes, golpes com efeitos de alteração de estados, trocar de Pokémon, entre outros. 

Como nãso foi utilizado nenhuma simulação, ao invés de simular estados futuros é consultado uma base de dados previamente gerada. Para isso, os agentes contam com dois modos: modo de aprendizagem e modo competitivo. No modo aprendizado os agentes escolhem sempre o grupo de ações menos explorado. No modo competitivo ele utiliza a base de dados estabelecida na fase de treino para escolher a melhor ação possível e também atualiza essa base de dados.
...
A figura \ref{fig:weaknessResistResult} mostra o objeto de retorno do método \textbf{getWeaknessAndResistances}. Como mostra a imagem é retornado um objeto com 5 vetores de \textbf{String} representando: imunidades (tipo de golpe que o Pokémon é imune), super resistências (tipo de golpe que é não muito efetivo contra Pokémon), resistências (tipo de golpe que é não efetivo contra o Pokémon), fraquezas (tipo de golpe que é efetivo contra Pokémon) e super fraqueza (tipo de golpe que é muito efetivo contra Pokémon).
...
O algoritmo utilizado pelos dois agentes foi o UCT descrito em \ref{sec:uct}. Toda a árvore de jogadas é feita na base de dados gerada pelo agente. No modo treino a única diferença do UCT é a fase de seleção, nesse modo, é sempre escolhido o grupo menos testado.
...
A ideia de criação dos dois algoritmos é ver a diferença de desempenho entre eles. Por isso, foi utilizado o modo \textit{Battle Factory} que, competitivamente falando, são modos menos aleatórios. A decisão de utilizar o modo \textit{Battle Factory} ao \textit{OU Battle} é o fato que no modo \textit{OU Battle} é necessário montar um time previamente e, para isso, é necessário um grande conhecimento de composição de times, um time mal escolhido pode levar um jogador a derrota independente de quão bem ele jogue.
