...
O método \textbf{myCreateBattle} recebe dois argumentos. O primeiro é chamado de \textbf{preview}, se \textbf{true}, ele define que o primeiro turno da batalha será para escolha do Pokémon inicial, se \textbf{false}, a ordem do vetor do time decidirá qual Pokémon irá começar na batalha. O segundo argumento é um vetor de times, a primeira posição do vetor é o time 1 que é um vetor de objetos Pokémon, a segunda posição do vetor é o time 2 que também é um vetor de objetos Pokémon.

Não é necessário um vetor completo de times (seis Pokémon em cada time) para criar uma batalha, ou seja, é possível simular um jogo onde cada time só tem um Pokémon (na imagem \ref{fig:showdownSimulation1} são criados dois times com apenas 3 Pokémon). Também não é necessário preencher todos os atributos do objeto Pokémon para criar uma batalha.
...
\begin{itemize}
	...
	\item \textbf{currentRequest}. É o tipo de turno. Existem três possíveis valores para essa propriedade:
		...
		\subitem \textbf{teampreview}. Acontece apenas no primeiro turno em batalhas criadas parâmetro \textbf{preview} com valor verdadeiro.
	...
	\item \textbf{p1} e \textbf{p2}. Objetos com todas as informações do jogador1 e jogador2 respectivamente da simulação.
	
\end{itemize}

Depois de criada a batalha ainda é possível alterar valores do objeto de batalha para melhor refletir a situação atual. Algumas propriedades que normalmente são necessários fazer ajustes: quantidade de HP atual de cada Pokémon, adicionar estados especiais de batalhas.
...
A figura \ref{fig:showdownSimulation5} mostra um exemplo de realização de ações os jogadores. Como a imagem mostra não é necessário nenhum comando para o turno avançar, assim que todos os jogadores façam suas escolhas o turno é avançado automaticamente. Existem situações em que apenas um dos dois jogadores tem que responder como a figura exemplifica.
...
Utilizando as APIs desenvolvidas foram criados 4 agentes para testar os diferentes modos de jogo e validar algumas técnicas. Foram criados 2 agentes para \textbf{Random Battle} e 2 agentes para \textbf{Battle Factory}. Nesta seção serão mostradas as técnicas utilizadas em cada agente e como ele foi empregado para o problema proposto.

Nos 4 agentes foram utilizadas técnicas de MCTS (explicados em \ref{sec:MCTS}), em alguns casos com modificações para se adequar ao teste proposto.
...
Para realizar esse aprendizado, cada agente conta com dois modos: modo de aprendizagem e modo competitivo. No modo aprendizado os agentes escolhem sempre o grupo de ações menos explorado. No modo competitivo ele utiliza a base de dados estabelecida na fase de treino para escolher a melhor ação possível, porém, ainda alimentando a base de dados.

Ambos os agentes utilizam a mesmo algoritmo. A única diferença entre os dois um dos agentes é que um dos agentes efetua o treinamento contra humanos (o agente será chamado \textbf{humanRandomAgent} ) e o outro contra um terceiro agente cópia desse agente no modo competitivo (o agente será chamado \textbf{botRandomAgent} ).

\subsubsection{Implementação dos agentes para Random Battle}

Para diminuir a largura da árvore de decisões e adicionar algum conhecimento de jogo durante as escolhas foi utilizada a técnicas de Grupos de Movimentos (detalhada em \ref{sec:GruposMovimentos}). 
...
\begin{itemize}
	...
	\item \textbf{Grupo A}. Golpe que são efetivos ou muito efetivos (\ref{sec:golpesPokemon}) contra o Pokémon adversário.
...
A figura \ref{fig:weaknessResistResult} mostra o objeto de retorno do método \textbf{getWeaknessAndResistances}. Como mostra a imagem é retornado um objeto com 5 vetores de \textbf{String} representando: imunidades (tipo de golpe que é imune ao Pokémon), super resistências (tipo de golpe que é não muito efetivo ao Pokémon), resistências (tipo de golpe que é não efetivo), fraquezas (tipo de golpe que é efetivo ao Pokémon) e super fraqueza (tipo de golpe que é muito efetivo ao Pokémon).
...
O algoritmo utilizado pelos dois agentes foi o UCT descrito em \ref{sec:uct}. A simulação é feita na base de dados gerada pelo agente. No modo treino a única diferença do UCT era a fase de seleção durante o modo treino, que era sempre escolhido o grupo menos testado.
...
A ideia de criação dos dois algoritmos é ver a diferença de desempenho entre eles. Por isso foi utilizado o modo \textit{Battle Factory} que, competitivamente falando, são modos menos aleatórios.
...
O objeto \textbf{factory} é muito importante nesses agentes, uma vez que não é conhecida qual customização o adversário utilizou em cada Pokémon, com um pouco de informação sobre o Pokémon já é possível estimar qual customização foi utilizada. Entre outras palavras, para fazer a simulação é pego todas as informações que são conhecidas durante a batalha sobre aquele Pokémon e é escolhido o \textit{set} que mais se assemelha a essas informações.

\subsubsection{Implementação dos agentes para Battle Factory}

O primeiro agente utiliza o algoritmo DUCT (exposto em \ref{sec:duct}) esse agente é chamado de \textbf{ductAgent}. O outro utiliza UCT sem modificações chamado de \textbf{uctAgent}.

Com a API de simulação não existe nenhuma base de dados de treinamento, ou seja, cada jogada é simulada em memória e a simulação é feita até o fim de jogo (como o algoritmo de MCTS define).

Os dois agentes fazem os 4 passos do MCTS (seleção, expansão, simulação e retropropagação) enquanto o temporizador de batalha tenha mais de 30 segundos. Desse modo os primeiros turnos de jogo, onde a altura da árvore e largura é muito maior, o agente tem 2 minutos para processar e, no decorrer dos demais turnos o agente tem entre 20 e 30 para estimar a melhor jogada (a quantidade de tempo ganha por turno é detalhada em \ref{sec:temporizadorBatalha}).

Cada nó da árvore de busca contém uma cópia da batalha inteira. Para isso, toda vez que um nó é criado (fase de expansão do MCTS) é feito uma cópia completa do time dos dois jogadores. 
...
Os métodos \textbf{createSimulationTeam1}, \textbf{createSimulationTeam2} e \textbf{applyCurrentStatus} estão todos na API \textbf{showdownSimulation}. O método \textbf{createSimulationTeam1} e \textbf{createSimulationTeam2} criam uma cópia dos times dos jogadores 1 e 2 respectivamente, já o método \textbf{applyCurrentStatus} é utilizado após o método \textbf{myCreateBattle} para aplicar a situação atual de cada Pokémon. Na figura \ref{fig:createCurrentSimulation} é mostrado dois modos diferente de como utilizar esses métodos. No primeiro modo (linha 2 até linha 10) é criado uma cópia do estado atual da batalha, ou seja, da que está acontecendo no navegador, para isso a API \textbf{showdownSimulation} utiliza a API \textbf{showdownAI}. No segundo modo (linha 14 até linha 22) é necessário passar mais um argumento para os métodos que é uma batalha simulada, ou seja, quando um nó diferente do nó raiz é expandido, esse modo é utilizado.
