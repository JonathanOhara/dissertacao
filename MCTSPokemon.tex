...
Em adição da API de \`{gameplay} foi desenvolvido uma API para simular situações e, ações em deterimanado cenários. Isso pode ser feito pois o código fonte do Pokémon Showdown! (servidor e cliente) é aberto.
...
A primeira grande função dessa API é criar uma batalha completa. A imagem \ref{fig:showdownSimulation1} mostra a utilização da API para criar uma batalha.
...
Não é necesseário um vetor completo de times para criar uma batalha, ou seja, é possível simular um jogo onde cada time só tem um Pokémon (na imagem \ref{fig:showdownSimulation1} são criados dois times com apenas 3 Pokémon). Também não é necessário preencher todos os atributos do objeto Pokémon para criar uma batalha.

A imagem \ref{fig:showdownSimulation2} mostra dois exemplos de funções que criam objetos Pokémon com todos atributos para criação de batalha.
...
\begin{itemize}
	...
	\item \textbf{currentRequest}. É o tipo de turno. Existem três possívels valores para essa propriedade:
	...
	\item \textbf{log}. Um vetor de \textit{String} com todas as mensagens do Pokémon Showdown sobre o que está acontecendo na simulação. Informações sobre composição de cada time, turno atual, movimentos utilizados, entre outras informações sobre a batalha estão disponiveís nessa propriedade.
	
	\item \textbf{p1} e \textbf{p2}. Objetos com todas as informações do jogador1 e jogador2 respectivamente da simulação.
	
\end{itemize}

Após criada a batalha ainda é possível alterar valores do objeto de batalha para melhor refletir a situação atual. Algumas propriedades que normalmente são necessarios fazer ajustes: quantidade de HP atual de cada Pokémon, adicionar estados especiais de batalhas.
...
\begin{itemize}
	...
	\item \textbf{chooseSwitch}. Esse método recebe como argumento um inteiro com valor de 1 a 6 relacionado para qual Pokémon será efetuado a trca.
