...
Árvores de decisão é uma forma muito clara e natural de representar uma cadeia de decisões e suas consequências. Segundo \cite{Millington:2009:AIG:1795711} árvores de decisão são rápidas, fáceis de implementar e simples de entender. Algumas vantagens dessa técnica são a fácil construção e a modularização.

Um modo muito comum de representar árvores de decisão para estratégias simples é o fluxograma. Segundo \cite{Millington:2009:AIG:1795711} as árvores de decisões representadas por fluxogramas têm normalmente duas respostas (sim e não, por exemplo). Na imagem \ref{fig:fluxogramaInimigos} é mostrado um diagrama simples de estratégia para um inimigo, nesse fluxograma o personagem patrulha o ambiente até achar um inimigo, ao ver um inimigo o personagem se aproxima se necessário e o ataca.
...
Um exemplo bem claro de FSM em jogos é o fantasma do clássico jogo do PacMan. No jogo o fantasma começa dentro de uma caixa (caixa de renascimento) por um determinado tempo (estado "esperando para nascer"), quando o tempo acaba ele entra no estado "caçando" indo atrás do PacMan, caso o PacMan coma uma bola de energia o fantasma entra no modo "sendo caçado" permitindo o PacMan derrotar ele, caso o temporizador da bola de energia se esgote o fantasma volta para o estado "caçando", caso o PacMan colida com o fantasma durante o estado "sendo caçado" o fantasma entra no estado "morto" e vai em direção da caixa de renascimento, ao chegar à caixa ele volta para o estado "esperando para nascer", a figura \ref{fig:fantasmaPacManFSM} ilustra essa FSM.

\begin{figure}[H]
\centering
\includegraphics[width=12cm]{figures/pacManFSM.png}
\caption[Fantasma PacMan FSM]{Fantasma PacMan FSM.}
\label{fig:fantasmaPacManFSM}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{Discreto}: discreto ou contínuo. Jogos discretos são os jogos cujo estado das variáveis são imediatamente alterados após uma ação ser executada. Jogos contínuos são jogos cujo uma ação pode refletir em muitos estados futuros.
	
\end{itemize}

Ainda segundo \cite{browne12asurvey} jogos com dois jogadores que são: soma-zero, de informação perfeita, determinísticos, discretos e sequenciais, podem ser descritos como jogos combinatórios. Essa definição inclui jogos como: Go, Xadrez e Jogo da Velha, assim como muitos outros.

A figura \ref{fig:arvorejogosjogodavelha} mostra uma árvore reduzida do jogo da velha. Na imagem, os nós ou estados são representados pelo estado do tabuleiro, as arestas representam a ação escolhida pelo jogador, as recompensas são atribuídas nos estados finais do jogo por positivo infinito (no caso de vitória do jogador "O") e infinito negativo (no caso de vitória do jogador "X").

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/gameTree.png}
\caption[Árvore de jogos]{Árvore de jogos (Jogo da Velha).}
\label{fig:arvorejogosjogodavelha}
\end{figure}

\subsection{Algoritmo minimax}
\label{sec:minimax}

O algoritmo \textit{minimax} é comumente utilizado em jogos de dois jogadores e baseados em turnos. Muitos jogos envolvem uma estrutura de recompensas em que, apenas as recompensas obtidas em estados terminais (jogadas que definem quem é o vencedor) descrevem com precisão o quão bem cada jogador está se saindo (\cite{browne12asurvey}). O algoritmo tenta minimizar a possível perda máxima, para isso ele analisa todos os possíveis movimentos futuros de cada jogador. Segundo os autores \cite{campbell1983comparison}, o algoritmo de \textit{minimax} funciona do seguinte modo: o algoritmo assume que existem dois jogadores chamados $Max$ e $Min$, e atribui um valor para cada nó dentro da árvore do jogo (e um particular para o nó raiz) do seguinte modo: nós folhas ou terminais propagam o valor \textit{minimax} recursivamente para todos os outros nós (passando para seu ancestral direto). Se um nó não terminal $p$ do jogador $Max$ for escolhido, então o valor de $p$ é o máximo valor dos filhos de $p$. Similarmente, se for o turno do jogador $Min$ será escolhido o menor valor dos sucessores de $p$.

O algoritmo básico do \textit{minimax} é ilustrado no Algoritmo \ref{alg:minimax}. No pseudocódigo, \textbf{node} se refere ao nó atual da árvore. Na primeira vez que \textbf{Minimax} é chamado o valor passado é passado o nó que representa o estado atual do jogo, no caso a raiz da árvore. A variável \textbf{depth} representa a profundidade do nó que está sendo analisado na árvore e, a cada passo do algoritmo esse valor é decrementado em um até chegar a zero, inicialmente o valor de \textbf{depth} é a altura da árvore. Por fim, \textbf{maxPlayer} representa de qual jogador o nó está sendo analisado, os possíveis valores de \textbf{maxPlayer} são TRUE (caso seja o jogador Max) e FALSE (caso seja o jogador Min), repare que a cada passo da recursão ele troca o valor da variável (de TRUE para FALSE e, de FALSE para TRUE).

\begin{algorithm}[H]
\caption{Algoritmo Minimax}
\label{alg:minimax}
\begin{algorithmic}[1]
\Procedure{Minimax}{$node, depth, maxPlayer$}
...
\label{sec:podaAlphaBeta}

Uma estratégia para reduzir a quantidade de nós analisados é utilizar técnicas de poda de árvores. A poda de árvore mais conhecida na literatura é a chamada \textit{alpha-beta}. Segundo \cite{knuth1976analysis} essa técnica é usada geralmente para aumentar a velocidade de busca sem perder informação. Nessa técnica são ignorados os nós e suas subárvores de jogadas incapazes de apresentar melhores valores do que movimentos já conhecidos. Durante a análise das jogadas são definidas duas variáveis \textit{alpha} e \textit{beta}. \textit{Alpha} representa o valor máximo que o jogador Max pode fazer e \textit{beta} a pontuação mínima do jogador Min. A cada avaliação de nó esses limites são verificados, caso o valor da avaliação não estiver entre esses limites o nó e todas suas subárvores são removidas (podadas). 

O procedimento do alpha-beta pode ser dado pelo Algoritmo \ref{alg:alphabeta}. As variáveis \textbf{node}, \textbf{depth} e \textbf{maxPlayer} apresentam o mesmo comportamento do algoritmo \textit{minimax}. Já \textit{alpha} e \textit{beta} representam os limites superiores e inferiores respectivamente, os valores iniciais para essas variáveis são $\alpha = -\infty$ e $\beta = +\infty$ desse modo ao encontrar o primeiro valor de heurística esses valores serão substituídos ( ver \ref{line:max} e \ref{line:min} ).

\begin{algorithm}[H]
\caption{Algoritmo alphabeta}
\label{alg:alphabeta}
\begin{algorithmic}[1]
\Procedure{alphabeta}{$node, depth, maxPlayer, alpha, beta$}
\If {$depth = 0$ \textbf{OR} node is terminal} 
	\State \Return heuristic
\EndIf
\If {$maxPlayer$}
	...
	\State $bestValue\gets -\infty$
	\For{each child \Pisymbol{psy}{206} $node$ }
		\State $bestValue\gets $\Call{max}{bestValue, \Call{alphabeta}{child, depth -1, FALSE, alpha, beta}}
		\State $alpha\gets \Call{max}{alpha, value}$ \label{line:max}
		\If {$beta \leqslant alpha$} 
			\State \textbf{break} \Comment{Corte beta}
		\EndIf
	\EndFor
...
\Else
	...
	\State $bestValue\gets +\infty$
	\For{each child \Pisymbol{psy}{206} $node$ }		
		\State $bestValue\gets $\Call{min}{bestValue, \Call{alphabeta}{child, depth -1, TRUE, alpha, beta}}
		\State $beta\gets \Call{min}{beta, bestValue}$ \label{line:min}
		\If {$beta \leqslant alpha$} 
			\State \textbf{break} \Comment{Corte alpha}
		\EndIf
	\EndFor
...
Em jogos com grande quantidade de nós na árvore (altura e largura), avaliar toda a árvore até um nó folha pode requerer muitos recursos (tempo e memória), um exemplo desse tipo de jogo é o xadrez, devido ao grande número de movimentos que se pode fazer em um turno e, a grande quantidade de turnos que um jogo pode durar, é inviável realizar uma busca completa até os nós folhas.
...
onde $l$ é o lado do quadrado. Já a área do círculo é dada pela seguinte equação:
...
Por não exigir muito conhecimento do domínio, ela funciona muito bem em jogos onde não existe uma função de avaliação de nós intermediários muito confiável, como no trabalho de \cite{brugmann1993monte} onde o MCE foi aplicado a um tabuleiro de 9x9 do jogo Go. A grande desvantagem dessa técnica é a quantidade de tempo que ela leva para avaliar um determinado nó, pois precisa chegar até um nó folha, fazendo a técnica eficaz apenas em árvores com profundidade de nós baixa. Uma derivação direta dessa abordagem é a árvore de busca de Monte Carlo.

\subsection{Algoritmo de busca de Monte Carlo}

Por décadas, a busca alpha-beta tem sido a abordagem padrão usada em jogos de soma-zero de dois jogadores como xadrez, damas entre outros. Muitas melhorias foram propostas para essa solução, porém em alguns jogos onde é difícil construir uma função de avaliação precisa (e.g., Go) a abordagem alpha-beta foi mal sucedida (\cite{Winands2008}).

O algoritmo de árvore de busca de Monte Carlo ganhou muita notoriedade por apresentar uma alta competitividade ao enfrentar os melhores jogadores do jogo Go. Esse jogo é particularmente desafiador para os algoritmos de busca em árvore, pois tem alto fator de ramificação, tem uma árvore profunda, e não existe nenhuma função de validação de heurística  confiável para nós não terminais (\cite{browne12asurvey}). Ainda segundo os autores, nos últimos anos o algoritmo de busca de Monte Carlo tem alcançado grande sucesso em muitos jogos específicos, jogos generalistas, complexos planejamentos do mundo real, otimização e controle de problemas, e se tornou uma importante ferramenta do pesquisador de inteligência artificial.
...
O algoritmo MCTS pode ser dado por Algoritmo \ref{alg:mcts}. A variável \textbf{currentNode} representa o nó do estado a analisar. O método \Call{BestChild}{node} escolhe o melhor nó filho. O conceito de melhor filho varia entre as diferentes implementações do MCTS. O algoritmo continua buscando um melhor resultado até que algum limite seja alcançado (\textbf{within computational budget}), geralmente é definido por tempo ou número de iterações.
...
\Procedure{MCTS}{$currentNode$}

	\While{within computational budget}
		\State $selectedNode\gets$ \Call{Selection}{$currentNode$}
		\State $expasionNode\gets$ \Call{Expasion}{$selectedNode$}
		\State $resultValue\gets$ \Call{Simulation}{$expasionNode$}
		\State \Call{Backpropagation}{$expasionNode$, $resultValue$}
	\EndWhile
	
	\State \Return \Call{BestChild}{currentNode}
\EndProcedure

\end{algorithmic}
\end{algorithm}

O algoritmo para fazer a seleção do nó para ser analisado pode ser definido por Algoritmo \ref{alg:mctsselection}. Repare que o método busca um \textbf{node} até que ele não tem filhos, dentro do laço \ref{line:selectionloop} a referência de \textbf{node} é trocado de acordo com a política de árvore ou \textbf{TreePolicy}.

\begin{algorithm}[H]
\caption{Algoritmo MCTS - método Selection}
\label{alg:mctsselection}
\begin{algorithmic}[1]

\Procedure{Selection}{$node$}
	\While{ $node.childs > 0$ } \label{line:selectionloop}
		\State $node\gets$ \Call{TreePolicy}{$node$}
	\EndWhile	
...
O algoritmo a simulação do nó pode ser definido por Algoritmo \ref{alg:mctssimulation}. A entrada do método é o \textbf{node} resultado da função de expansão. A partir desse nó são executadas (\textbf{Execute}) ações (\textbf{action}) até que uma ação resulte em fim de jogo (\textbf{action.isEndOfGame}), caso não resulte em fim de jogo é lido as ações disponíveis (\textbf{ReadAvaliableActions}) e entre elas escolhido uma ação (pode ser segundo alguma heurística ou, como no exemplo \textbf{ChooseRandomAction} ser aleatória), caso resulte em fim de jogo o valor da recompensa do fim de jogo é retornado.

\begin{algorithm}[H]
\caption{Algoritmo MCTS - método Simulation}
\label{alg:mctssimulation}
\begin{algorithmic}[1]

\Procedure{Simulation}{$node$}
	\State $action\gets node.action$
	\While{ \textbf{not} $action.isEndOfGame$}
		\State \Call{Execute}{$action$}
		\State $actions\gets$ \Call{ReadAvaliableActions}{ }
		\State $action\gets$ \Call{ChooseRandomAction}{action}
	\EndWhile
	
	\State $value\gets$ $\frac{1}{2}$
	\If{ $action.isVictory$ }
		\State $value\gets$ $1$
	\ElsIf{ $action.isDefeat$ }
		\State $value\gets$ $0$
	\EndIf
...
\subsubsection{Upper Confidence Bound for Trees (UCT)}
\label{sec:uct}
...
onde $j$ é a escolha que está sendo analisada (no exemplo uma máquina caça-níquel específica), $\overline{x}_{j}$: a média da recompensa paga pela máquina $j$,$n_{j}$: a quantidade de vezes que foi jogado na máquina $j$ e $n$: A soma de quantas vezes foi jogado em cada máquina.

Nessa equação fica claro como estão distribuídos os fatores exploração ($\sqrt{\frac{2 \ln n}{n_{j}}}$) e aprofundamento ($\overline{x}_{j}$). A imagem \ref{fig:exploreandexploitinequation} ilustra essa separação.
...
Baseado no algoritmo MCTS previamente explicado, algumas alterações são necessárias para implementar o UCT. O Algoritmo base para o UCT pode ser definido por Algoritmo \ref{alg:uctmain}.

\begin{algorithm}[H]
\caption{Algoritmo UCT}
\label{alg:uctmain}
\begin{algorithmic}[1]
\Procedure{UCT}{$currentNode$}
	\While{within computational budget}
		\State $selectedNode\gets$ \Call{Selection}{$currentNode$}
		\State $expasionNode\gets$ \Call{Expansion}{$selectedNode$}
		\State $resultValue\gets$ \Call{Simulation}{$expasionNode$}
		\State \Call{Backpropagation}{$expasionNode$, $resultValue$}
	\EndWhile
	
	\State \Return \Call{UCB1}{currentNode}
\EndProcedure
\end{algorithmic}
\end{algorithm}

No fim do algoritmo a escolha do melhor filho é realizado pelo de UCB1 (também utilizado na seleção) descrito em Algoritmo \ref{alg:uctucb1}.
...
\Procedure{UCB1}{$node$}
	\State $bestValue\gets -\infty$ 
	\State $bestChild\gets$ \textbf{null}
	\For{each child \Pisymbol{psy}{206} $node$ }
		\State $value\gets child.value + C \sqrt{\frac{2 \ln node.visits}{child.visits}}$
		\If{ $value > bestValue$ }
			\State $bestValue\gets value$
			\State $bestChild\gets child$
		\EndIf
	\EndFor
	
	\State \Return $bestChild$
\EndProcedure
\end{algorithmic}
\end{algorithm}

O método selecionar do UCT é descrito em Algoritmo \ref{alg:uctselecionar}. A política de árvore utilizada é o próprio algoritmo de UCB1 (Algoritmo \ref{alg:uctucb1}).

\begin{algorithm}[H]
\caption{Algoritmo UCT - Selecionar}
\label{alg:uctselecionar}
\begin{algorithmic}[1]
\Procedure{Selecionar}{$node$}
	\While{ $node.filhos > 0$ }
		\State $node\gets$ \Call{UCB1}{$node$}
	\EndWhile
...
O pseudocódigo do método retroproapagar é definido por Algoritmo \ref{alg:uctbackpropagation}. No método ele atualiza tanto o valor (\textbf{value}) do nó quanto a quantidade de visitas (\textbf{visits}) até o nó raiz, ou seja, até que o pai (\textbf{father}) seja nulo.

\begin{algorithm}[H]
\caption{Algoritmo UCT - Retropropagar}
\label{alg:uctbackpropagation}
\begin{algorithmic}[1]
\Procedure{Backpropagation}{$node$, $value$}
	\State $node.visits\gets node.visits + 1$ 
	\State $node.value\gets node.value + value$

	\If{ node.father \textbf{not} null }
		\State \Call{Backpropagation}{$node.father$, $node$}
	\EndIf
...
\subsection{Aprimoramentos para o MCTS}

Existem muitos aprimoramentos que alteram em partes o funcionamento do MCTS para aprimorar resultados ou para se adaptar a diferentes tipos de jogos. A seguir veremos algumas importantes técnicas e modificações.

\subsubsection{Urgência de primeira jogada}

A técnica urgência de primeira jogada (do inglês First Play Urgency - FPU) de \cite{gelly2006exploration} sugere melhorias para a fase de seleção do algoritmo. Segundo os autores o UCT não dá nenhuma solução para escolher entre movimentos que ainda não foram explorados, ou seja, a equação UCB1 (\ref{eq:ucb1}) não é valida em nós com nenhuma visita. Em seu trabalho o autor sugere iniciar a avaliação dos nós com o valor de 10000, e após serem visitados ao menos uma vez a sua avaliação ocorre normalmente pela fórmula de UCB1.

\subsubsection{Grupos de movimentos}
\label{sec:GruposMovimentos}
...
\subsection{Jogos de movimentos simultâneos}

Jogos de movimentos simultâneos são jogos onde ambos os jogadores escolhem suas jogadas ao mesmo tempo e, após os dois terem escolhidos seus movimentos, as duas escolhas são reveladas juntas.	Os trabalhos \cite{lanctot2013monteI}, \cite{lanctot2013monte2} e \cite{lisy2013convergence} estudam a aplicação do MCTS para jogos com movimento simultâneos. Nesses trabalhos as principais técnicas estudadas são: UCT desacoplado e Exp3.

%, Regret Matching e Outline Outcome Sampling.%

\subsubsection{UCT desacoplado}
\label{sec:duct}

O UCT desacoplado (do inglês Decoupled UCT - DUCT) foi proposto por \cite{finnsson2007cadia} para a competição General Game Playing para resolver problemas de jogos com movimentos simultâneos. No DUCT cada nó representa uma escolha para cada jogador, ou seja, os nós que estão na mesma altura na árvore representam todas as combinações de movimentos dos dois ou mais jogadores, esses nós são chamados de articulações. Cada jogador mantém uma variáveis próprias para representar a recompensa (número de vitórias e derrotas) e visitas para cada nó (\cite{lanctot2013monteI}), ou seja, como o trabalho de \cite{lanctot2013monte2} diz: "na seleção, DUCT aplica o algoritmo UCB1 com as estatísticas da perspectiva de cada jogador independentemente". 

A imagem \ref{fig:ductcomparing} compara uma árvore MCTS de jogos sequenciais (Árvore a esquerda) com uma árvore DUCT (Árvore a direita). Na imagem é ilustrado o mesmo cenário onde o jogador 1 tem o conjunto de movimentos {A, B, C} e o jogador 2 tem o conjunto de movimentos {a, b, c}. Esses nós com a combinação de escolhas entre os $N$ jogadores é chamado de articulação (do Inglês joint).

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/ductcomparing.pdf}
\caption[UCT e DUCT]{UCT e DUCT.}
\label{fig:ductcomparing}
\end{figure}

Após a simulação de ambos os jogadores terminar, a melhor jogada é escolhida de dois diferentes modos \cite{lanctot2013monte2}. O primeiro, DUCT(max) seleciona o movimento que teve mais visitas. Já DUCT(mix) normaliza a quantidade de visitas de cada movimento e escolhe o movimento de acordo com a distribuição.

Uma das grandes vantagens dessa abordagem é a facilidade de paralelizar o código, uma vez que no DUCT é rodado o algoritmo UCB1 independente para cada jogador como mostra no trabalho de \cite{finnsson2007cadia}.

\subsubsection{MCTS Múltiplo}

O MMCTS (do Inglês Multiple Monte Carlo Tree Search) foi proposto por \cite{DBLP:journals/corr/abs-1102-1580} para jogos com informações imperfeitas. Nesse algoritmo é criada uma árvore para cada jogador e essas árvore são buscadas e atualizadas simultaneamente. Em sua pesquisa \cite{DBLP:journals/corr/abs-1102-1580} aplicou essa técnica para resolver o problema do jogo da velha fantasma. Diferente do tradicional jogo da velha, nesse jogo os jogadores não podem ver a ação do oponente, o único modo de um jogador saber onde o outro fez uma jogada é, se ao tentar fazer uma jogada a posição já estar preenchida pelo adversário, desse modo o jogador é avisado que não pode fazer a jogada pois o local já está preenchido.

No MMCTS a seleção é feita através do algoritmo chamado Exp3 (do Inglês Exponential-weight algorithm for Exploration and Exploitation) que foi proposto por \cite{auer1995gambling} para também resolver o problema dos caça-níqueis. O algoritmo está ilustrado em \ref{alg:exp3}. No pseudocódigo $\gamma \in [0;1]$ irá fazer o controle entre explorar novas ações e validar resultados de ações bem avaliadas anteriormente, $weights$ é a matriz com o peso que cada ação tem em cada turno, é preciso iniciar os valores de peso para o primeiro turno do jogo, que pode ser feito com um valor igualitário ou sofrer influência de alguma heurística conhecida (método $InitializeWeights$), após isso a matriz de probabilidade $p$ de escolha de escolha de uma ação $i$ no turno $t$ é definido baseado na distribuição de pesos entre todas as escolhas. Após isso, uma ação $c$ é escolhida aleatoriamente baseada na distribuição  de $p_t$ (através do método $GetRandomActionAccordingTheProbabilty$). Com a ação escolhida calculamos a recompensa observada $reward_tc$ da ação $c$ e a recompensa estimada $estimatedReward_t^c$. Por fim $weights_{t+1}$ é atualizado com o mesmo valor $weights_t$ para todas as ações não escolhidas e, para a ação escolhida, o peso $weights_{t+1}^c$ da ação é atualizado com base na recompensa estimada.

\begin{algorithm}[H]
\caption{Algoritmo Exp3}
\label{alg:exp3}
\begin{algorithmic}[1]
\Procedure{Exp3}{}
	\State $gamma\gets \Call{Randomize}{0, 1}$
	\State $weights\gets \Call{InitializeWeights}{}$
	\State $k\gets \Call{CountNumberOfActions}{}$

	\For{each round $t$}
		\For{each action $i$}
			\State $p_t^i\gets (1 - \gamma) \frac{weights_t^i}{ (\sum\limits_{j=1}^{k} weights_t^j) } + \frac{\gamma}{k}$
		\EndFor

		\State $c\gets \Call{GetRandomActionAccordingTheProbabilty}{p_t}$
		\State $reward_t^c\gets \Call{GetActionReward}{c}$
		\State $estimatedReward_t^c\gets \frac{x_t^c}{p_t^c}$
		
		\For{each action $i$}
			\State $weights_{t+1}^i\gets weights_t^i$
		\EndFor
		
		\State $weights_{t+1}^c\gets weights_t^c e^{\gamma \frac{estimatedReward_t^c}{k}}$
	\EndFor
