...
\begin{itemize}
	...
	\item \textbf{Dicreto}: discreto ou contínuo. Jogos discreto são os jogos cujo o estado das variáveis são imediatamente alterados após uma ação ser executada. Jogos conínuos são jogos cujo uma ação pode refletir em muitos estados futuros.
...
Jogos de movimentos simultâneos são jogos onde ambos os jogadores escolhem suas jogadas ao mesmo tempo e, após os dois terem escolhidos seus movimentos, as duas escolhas são reveladas juntas.	Os trabalhos \cite{lanctot2013monteI}, \cite{lanctot2013monteII} e \cite{lisy2013convergence} esutdam a aplicação do MCTS para jogos com movimento simultâneos. Nesses trabalhos as principais técnicas estudadas são: Decoupled UCT, Exp3, Regret Matching e Outline Outcome Sampling.

\subsubsection{UCT desacoplado}

O UCT desacoplado (do inglês Decoupled UCT - DUCT) foi proposto por \cite{finnsson2007cadia} para a competição General Game Playing para resolver problemas de jogos com movimentos simultâneos. no DUCT cada nó representa uma escolha para cada jogador, ou seja, os nós que estão na mesma altura na árvore representam todas as combinações de movimentos dos dois ou mais jogadores, esses nós são chamados de articulações. Cada jogador mantém uma variáveis próprias para representar a recompensa (número de vitórias e derrotas) e visitas para cada nó (\cite{lanctot2013monteI}), ou seja, como o trabalho de \cite{lanctot2013monteII} diz: "na seleção, DUCT aplica o algoritmo UCB1 com as estastísticas da perspectativa de cada jogador independentemente". 

A imagem \ref{fig:ductcomparing} compara uma árvore MCTS de jogos sequenciais (Árvore a esquerda) com uma árvore DUCT (Árvore a direita). Na imagen é ilustrado o mesmo cenário onde o jogador 1 tem o conjunto de movimentos {A, B, C} e o jogador 2 tem o conjunto de movimentos {a, b, c}. Esses nós com a combinação de escolhas entre os $N$ jogadores é chamado de articulação (do Inglês joint).

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/ductcomparing.pdf}
\caption[DUCT e UCT]{DUCT e UCT.}
\label{fig:ductcomparing}
\end{figure}

Após a simulação de ambos os jogadores terminar a melhor jogada é escolhida de dois diferentes modos \ref{lanctot2013monteII}. O primeiro, DUCT(max) seleciona o movimento que teve mais visitas. Já DUCT(mix) normaliza a quantidade de visitas de cada movimento e escolhe o movimento de acordo com a distribuição.
